...
class Server():
  ...
  def checkBound(self, error):
    if self.num_connects_last_interval > 5 and self.ddos_detected == 0: #self.avg_connects_per_interval + error
      print "DDoS ATTACK WARNING...\n<CAPTCHA MODE ENABLED>"
      self.ddos_detected = 1
      self.captcha_mode = 1
      self.ddos_count += 1
    elif self.num_connects_last_interval > 100 and self.ddos_detected > 0: #self.avg_connects_per_interval + error
      print "DDoS ATTACK DETECTED! ERROR...\n<CAPTCHA MODE ENABLED>"#, self.ddos_detected
      self.ddos_detected += 1
      self.captcha_mode = 1
      self.ddos_count += 1
